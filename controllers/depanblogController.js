...
const Depanblog = db.Depanblog;


const store = async (req, res) => {
    try {
        const { image, title, content } = req.body;
        const save = await Depanblog.create({ image, title, content });
...
const index = async (req, res) => {
    try {
        const result = await Depanblog.findAll({ attributes: ['image', 'title', 'content'] });
...
const show = async (req, res) => {
    try {
        ...
        const data = await Depanblog.findByPk(id, { attributes: ['image', 'title', 'content'] });
...
const update = async (req, res) => {
    try {
        const id = req.params.id;
        const depanblog = await Depanblog.findByPk(id);
        if (depanblog) {
            const {image, title, content} = req.body;
            await depanblog.update({image, title, content});
...
const destroy = async (req, res) => {
    try {
        const id = req.params.id;
        const kategori = await Depanblog.findByPk(id);
        if (kategori) {
